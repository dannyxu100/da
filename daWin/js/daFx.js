	...
	daFx.fnStruct = daFx.prototype = {
		...
		*/
		update: function() {
			...
			(daFx.step[this.prop] || daFx.step._default)( this );				//调用daFx.step绘制元素
		},
		...
		*/
		custom: function( from, to, unit ) {
			...
			t.elem = this.elem;																																		//保存一个元素的引用。删除和判断is:animated的时候用到,把timer和元素联系起来  
		...
		step: function( gotoEnd ) {
			...
			if ( gotoEnd || t >= (options.duration + this.startTime) ) {				 //如果gotoEnd为true或者已到了动画完成的时间.  
				...
				if ( options.duration == Infinity ) {																//默认的easing动画算法，不能用无穷值
					...
					n = t - this.startTime;																						//动画执行完还需要的时间
					this.state = n / options.duration;																//还需要时间的比例
