	...
	daCalendar.fnStruct = daCalendar.prototype = {
			...
			*/
			init: function( setting ){
				...
				this.cdrYear = da.firstValue( setting.year, this.cdrToday.year );					//默认加载当前年月的日历
				this.cdrMonth = da.firstValue( setting.month, this.cdrToday.month );
			...
			*/
			createToolBar: function( ){
			    ...
			    cdrBar.btFresh = doc.createElement( "a" );
					...
					cdrBar.btToday.className = this.setting.isSelector ? this.setting.css.barToday_slt : this.setting.css.barToday;
					cdrBar.btPrevious.className = this.setting.isSelector ? this.setting.css.barPrevious_slt : this.setting.css.barPrevious;
					cdrBar.btNext.className = this.setting.isSelector ? this.setting.css.barNext_slt : this.setting.css.barNext;
					cdrBar.spanTitle.className = this.setting.css.barTitle;
					cdrBar.btFresh.className = this.setting.isSelector ? "" : this.setting.css.barBt;
...
//					cdrBar.barLeft.insertBefore( cdrBar.btFresh );
					...
					if( this.setting.isSelector ){
							...
							if( this.setting.isSelector ){
								cdrBar.btFresh.style.cssText = 'display:inline-block;width:18px;height:18px;line-height:24px;text-align:center;color:#333;text-decoration:none; font:14px/1.5 arial, sans-serif; font-weight:bold;';
								cdrBar.btFresh.innerHTML = "X";
							}
							else
								cdrBar.btFresh.innerHTML = "刷";
					}
					else{
							cdrBar.btNext.href = "javascript:void(0);";
							cdrBar.barLeft.insertBefore( cdrBar.btNext );
			...
			*/
			bindEvent: function( ){
					var context = this,
							...
							nYear = parseInt(this.cdrYear),
							nPreMonth = parseInt(this.cdrMonth ) - 1,
							nNextMonth = parseInt(this.cdrMonth ) + 1,
							dateTmp = null,
							cdrObjTmp = null;
					...
					da( this.cdrBar.btPrevious ).bind( "click", function( evt ){											//“上一月”按钮自定义事件函数
						...
						if( false !== res ){
							dateTmp = new Date( nYear, nPreMonth );
