...
var daCanvas = (function(){
	...
	/**daPopMenu类构造函数
	*/
	var daCanvas = function( cvsSetting ){
		return new daCanvas.fnStruct.init( cvsSetting );
	};
	...
	daCanvas.fnStruct = daCanvas.prototype = {
		...
		init: function( cvsSetting ){
			cvsSetting = this.cvsSetting = da.extend( {}, this.cvsSetting, cvsSetting );

			if( _HasSVG )
				this.dc = daSVG( cvsSetting );
			else
				this.dc = daVML( cvsSetting );
