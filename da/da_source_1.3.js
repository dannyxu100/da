				...
				if ( typeof obj === "string" ) {										//如果传入的obj是字符串，可能会用到Sizzle选择器
					...
					if ( match && (match[1] || !context) ) {
						...
						if ( match[1] ) {
							...
							return da.merge( this.dom, obj );									//合并元素集合数组
		...
		*/
		remove: function( selector, keepData ) {
          ...
          if( da.browser.ie && elem.parentNode){
              var dustbin = document.getElementById("da_Dustbin");
              if( null === dustbin ){
              	dustbin = document.createElement('div');
              	dustbin.id = "da_Dustbin";
              	document.body.insertBefore( dustbin );
              }

              dustbin.insertBefore( elem );
              dustbin.innerHTML = '';
	...
	da.extend({
		...
		*/
		...
		/*
			...
			useCapture : 是否锁定 当前对象为事件捕获对象 范围
		*/
		eventRemove: function(objDOM, objEvents, useCapture) {
			 useCapture = useCapture || false;
...
//				});
			...
			daMaskFresh: function( maskWin ){
				...
				if( !objMask ) objMask = this.daMaskInit( maskWin );
...
//				else
	...
	//页面加载完毕
	da(function(){
			if( !da.daGetMask(window) ) da.daMaskInit(window);
	});
	...
	da.extend({
		...
		*/
		out: function( msg, color ){
			...
			var pushWin = da.getRootWin() || win,
					outDiv = pushWin.document.getElementById("daDebugOutDiv"),
					lineObj;
			...
			if( null === outDiv ){
				...
				pushWin.document.body.insertBefore( containerDiv, pushWin.document.body.firstChild );
				...
				closeDiv = doc.createElement("div");
			...
			if( da.isArray( msg ) ){
				...
				lineObj = doc.createElement("textarea");
				lineObj.value = "代码：\n" + msg;
