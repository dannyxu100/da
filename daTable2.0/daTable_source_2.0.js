	...
	daTable.fnStruct = daTable.prototype = {
		...
		*/
		addItem: function( ds ){
			...
			var tmpfmt, fldname, fldvalue, fldidx = 0,
				// arrHTML = [],
				record = null;
			...
			for (var i=0,len=ds.length; i < len; i++ ){									//行循环
				...
				tmpHTML = tmpHTML.replace(/\{[^\}]+\}/g, function( res, idx, target ){
					fldname = res.replace( /\{|_org|_raw|\}/g, "" );
					fldvalue = "{order}" == res 
						? context.curOrder : (record[fldname] || res);					//追加流水号
					
					return context.formatData( fldname, fldvalue, record, ds );
				}); 
				
				da(dataBody).append(tmpHTML);
