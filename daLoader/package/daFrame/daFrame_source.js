			...
			//滚动条拖拽平移
 			daDrag({
				window: win,
 				...
 				move: function( evt, srcObj, targetObj, oldPos, nowPos, prevMousePos, nowMousePos ){ 
					var minY = 1, 
							maxY = da(context.cntMainObj).height() - da(context.cntScrollV).height() - context.cntCorner;			//减去滚动条不能经过的角落处
 			...
 			daDrag({
				window: win,
 				...
 				move: function( evt, srcObj, targetObj, oldPos, nowPos, prevMousePos, nowMousePos ){
					var minX = 1,
							maxX = da(context.cntMainObj).width() - da(context.cntScrollH).width() - context.cntCorner;			//减去滚动条不能经过的角落处
		...
		*/
		bindWheel:function( objPad ){
			...
			objPad.daWheelObj = daWheel({
				target: objPad,
				before: function(){
			    maxY = da(context.cntBoxObj).height() - da(context.cntMainObj).height() - 4,	//可滚动最大高度 兼容ie 减去边框值-4
...
//					context.cntIframeObj = tmpFrame;
		...
		/**设置滚动条（显示）位置
		* params {Int|String} top 滚动条纵向位置
		* params {Int|String} left 滚动条横向位置
		*/
		scroll: function( top, left ){
			da( this.cntBoxObj ).css({ top: "-"+top, left: "-"+left });								//设置内容box位置
			
			var scrollTop = top * this.cntScale.h,
					scrollLeft = left * this.cntScale.w;

			da( this.cntScrollV ).css({ top: scrollTop, left: scrollLeft });
